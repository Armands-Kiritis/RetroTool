export const en = {
  common: {
    back: "Back",
    ok: "OK",
    loading: "Loading...",
    creator: "Creator",
    boardId: "Board ID",
    participants: "participants",
    shareLink: "Share Link",
    copySuccess: "Board link copied to clipboard!",
    archiveBoard: "Archive Board",
    deleteBoard: "Delete Board",
    unarchive: "Unarchive",
    archived: "Archived",
    archiveConfirm:
      "Are you sure you want to archive this board? It will become read-only and move to the archived section.",
    deleteConfirm:
      "Are you sure you want to permanently delete '{boardName}'? This action cannot be undone and all data will be lost.",
    deleteSuccess: "Board deleted successfully.",
    deleteFailed: "Failed to delete board. Please try again.",
    boardNotFound: "Board not found. Please check the board ID.",
    joinFailed: "Failed to join board. Please try again.",
    logout: "Logout",
  },
  auth: {
    login: "Login",
    signUp: "Sign Up",
    username: "Username",
    password: "Password",
    confirmPassword: "Confirm Password",
    loginDescription: "Sign in to access your retrospective boards",
    signUpDescription: "Create an account to get started",
    noAccount: "Don't have an account?",
    haveAccount: "Already have an account?",
    usernameRequirement: "At least 3 characters",
    passwordRequirement: "At least 6 characters",
    passwordMismatch: "Passwords do not match",
    loginFailed: "Login failed. Please check your credentials.",
    registrationFailed: "Registration failed. Please try again.",
  },
  nameEntry: {
    welcome: "Welcome!",
    enterName: "Enter your name to get started",
    yourName: "Your name",
    continue: "Continue",
  },
  boardSelection: {
    hello: "Hello, {name}!",
    createOrJoin: "Create a new retrospective board or join an existing one",
    createNew: "Create New Board",
    boardName: "Board name (e.g., Sprint 23 Retro)",
    createBoard: "Create Board",
    joinById: "Join by Board ID",
    boardIdPlaceholder: "Board ID (e.g., ABC123XY)",
    joinBoard: "Join Board",
    allBoards: "All Boards",
    archivedBoards: "Archived Boards",
    myBoards: "My Boards",
    myArchivedBoards: "My Archived Boards",
    showActive: "Show Active",
    showArchived: "Show Archived",
    showingArchived: "Showing Archived",
    showMy: "My Boards",
    showAll: "Show All Boards",
    searchBoards: "Search boards by name or creator...",
    noBoardsCreated: "No boards created yet",
    noArchivedBoards: "No archived boards",
    noMyBoards: "You haven't created any boards yet",
    noMyArchivedBoards: "You haven't archived any boards yet",
    noSearchResults: "No boards found matching your search",
    allBoardsLoaded: "All boards loaded",
    by: "by",
  },
  retroBoard: {
    archivedMessage: "This board is archived",
    archivedDescription:
      "Archived on {date} by {name}. You can view the content but cannot make changes. Any team member can unarchive this board using the button above.",
    glad: "😊 Glad",
    mad: "😠 Mad",
    sad: "😢 Sad",
    whatMadeYou: "What made you {emotion}?",
    addItem: "Add Item",
    editItem: "Edit Item",
    deleteItem: "Delete Item",
    deleteItemConfirm: "Are you sure you want to delete this item? This action cannot be undone.",
    hiddenItem: "Hidden item from {name}",
    revealed: "Revealed",
    revealItem: "Reveal Item",
    hideItem: "Hide Item",
    teamRetrospective: "Team Retrospective",
    save: "Save",
    cancel: "Cancel",
    editFailed: "Failed to edit item. Please try again.",
    deleteFailed: "Failed to delete item. Please try again.",
  },
  timer: {
    setTimer: "Set Timer",
    timerTitle: "Set Timer",
    timerDescription:
      "Set a timer for team members to add items to the board. When the timer ends, everyone will be notified.",
    minutes: "Minutes",
    startTimer: "Start Timer",
    timeUp: "Time's Up!",
    timeUpMessage: "The time for adding items to the board has ended.",
    timeUpDescription: "You can now proceed with discussing the items.",
  },
  language: {
    select: "Language",
    en: "English",
    lv: "Latvian",
    lt: "Lithuanian",
    no: "Norwegian",
  },
}
